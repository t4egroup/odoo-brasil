...
class WizardCartaCorrecaoEletronica(models.TransientModel):
    ...
    state = fields.Selection(
        [("drat", "Provisório"), ("error", "Erro")], string="Situação"
    )
    ...
    def valida_carta_correcao_eletronica(self):
        if len(self.correcao) < 15:
            raise UserError(_("Motivo de Correção deve ter mais de 15 caracteres"))
        if len(self.correcao) > 1000:
            raise UserError(_("Motivo de Correção deve ter menos de 1000 caracteres"))
    ...
    def send_letter(self):
            ...
            "eventos": [
                {
                    ...
                    "tpAmb": 2
                    if self.eletronic_doc_id.ambiente == "homologacao"
                    else 1,
        ...
        certificado = Certificado(
            cert_pfx, self.eletronic_doc_id.company_id.l10n_br_cert_password
        )
        ...
        if retorno.cStat == 128 and retorno.retEvento.infEvento.cStat in (135, 136):
            ...
            if retorno.cStat == 128:
                mensagem = "%s - %s" % (
                    retorno.retEvento.infEvento.cStat,
