...
class ExportNfe(models.TransientModel):
    ...
    emitter = fields.Selection(
        [("propria", "Própria"), ("terceiros", "Terceiros")], string="Emissão"
    )
    ...
    state = fields.Selection(
        [("init", "init"), ("done", "done")], "state", readonly=True, default="init"
    )
    ...
    def nfse_export(self):
        ...
        if self.emitter == "propria":
            ...
            search_vals.append(
                ("state", "in", ["cancel", "done", "denied", "imported"])
            )
        if self.edoc_type:
                ...
                danfe_report = self.env["ir.actions.report"].search(
                    [
                        (
                            "report_name",
                            "=",
                            "l10n_br_eletronic_document.main_template_br_nfe_danfe",
                        )
                    ]
                )
                report_service = danfe_report.xml_id
                danfe, dummy = self.env.ref(report_service).render_qweb_pdf(
                    [invoice.id]
                )
                report_name = safe_eval(
                    danfe_report.print_report_name, {"object": invoice}
                )
                filename = "%s.%s" % (report_name, "pdf")
                pdfs.append({"content": danfe, "name": filename})
        ...
        mod_obj = self.env["ir.model.data"].search(
            [("model", "=", "ir.ui.view"), ("name", "=", "view_wizard_export_nfe")]
        )
