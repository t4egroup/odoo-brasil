...
class EletronicDocument(models.Model):
    ...
    identifier = fields.Char(string="Identificador", readonly=True, states=STATE)
    ...
    # ------------ Retencoes ------------
    irrf_base_calculo = fields.Monetary(string="Base IRRF", readonly=True, states=STATE)
    irrf_valor_retencao = fields.Monetary(
        string="Valor IRRF", readonly=True, states=STATE
    )
    inss_base_calculo = fields.Monetary(string="Base INSS", readonly=True, states=STATE)
    ...
    valor_final = fields.Monetary(string="Valor Final", readonly=True, states=STATE)
    ...
    numero = fields.Integer(string="Número", readonly=True, states=STATE, copy=False)
    ...
    data_agendada = fields.Date(
        string="Data agendada", default=fields.Date.today, readonly=True, states=STATE
    )
    ...
    payment_term_id = fields.Many2one(
        "account.payment.term", string="Condição pagamento", readonly=True, states=STATE
    )
    fiscal_position_id = fields.Many2one(
        "account.fiscal.position", string="Posição Fiscal", readonly=True, states=STATE
    )
    ...
    #     readonly=True, states=STATE)
    valor_bruto = fields.Monetary(string="Valor Bruto", readonly=True, states=STATE)
    valor_frete = fields.Monetary(string="Total Frete", readonly=True, states=STATE)
    valor_seguro = fields.Monetary(string="Total Seguro", readonly=True, states=STATE)
    ...
    valor_bc_icms = fields.Monetary(string="Base ICMS", readonly=True, states=STATE)
    valor_icms = fields.Monetary(string="Total do ICMS", readonly=True, states=STATE)
    ...
    valor_icmsst = fields.Monetary(string="Total ST", readonly=True, states=STATE)
    valor_ii = fields.Monetary(string="Total II", readonly=True, states=STATE)
    valor_ipi = fields.Monetary(string="Total IPI", readonly=True, states=STATE)
    ...
    codigo_retorno = fields.Char(
        string="Código Retorno", readonly=True, states=STATE, tracking=True, copy=False
    )
    ...
    numero_nfe = fields.Char(string="Numero Formatado NFe", readonly=True, states=STATE)
    ...
    uf_veiculo = fields.Char(string="UF da Placa", size=2, readonly=True, states=STATE)
    rntc = fields.Char(
        string="RNTC",
        size=20,
        readonly=True,
    ...
    # Cobrança
    numero_fatura = fields.Char(string="No. Fatura", readonly=True, states=STATE)
    fatura_bruto = fields.Monetary(string="Valor Original", readonly=True, states=STATE)
    fatura_desconto = fields.Monetary(string="Desconto", readonly=True, states=STATE)
    ...
    sequencial_evento = fields.Integer(
        string="Sequêncial Evento", default=1, readonly=True, states=STATE, copy=False
    )
    recibo_nfe = fields.Char(
        string="Recibo NFe", size=50, readonly=True, states=STATE, copy=False
    )
    chave_nfe = fields.Char(
        string="Chave NFe", size=50, readonly=True, states=STATE, copy=False
    )
...
class EletronicDocumentLine(models.Model):
    ...
    # Florianopolis
    codigo_cnae = fields.Char(string="CNAE", size=10, readonly=True, states=STATE)
    # Paulistana
    ...
    frete = fields.Monetary(string="Frete", readonly=True, states=STATE)
    seguro = fields.Monetary(string="Seguro", readonly=True, states=STATE)
    desconto = fields.Monetary(string="Desconto", readonly=True, states=STATE)
    ...
    valor_bruto = fields.Monetary(string="Valor Bruto", readonly=True, states=STATE)
    valor_liquido = fields.Monetary(string="Valor Líquido", readonly=True, states=STATE)
    ...
    icms_cst = fields.Selection(
        CST_ICMS + CSOSN_SIMPLES, string="CST ICMS", readonly=True, states=STATE
    )
    icms_aliquota = fields.Float(
        string="Alíquota ICMS", digits="Account", readonly=True, states=STATE
    )
    ...
    icms_base_calculo = fields.Monetary(string="Base ICMS", readonly=True, states=STATE)
    icms_aliquota_reducao_base = fields.Float(
        string="% Redução Base ICMS", digits="Account", readonly=True, states=STATE
    )
    icms_valor = fields.Monetary(string="Valor ICMS", readonly=True, states=STATE)
    icms_valor_credito = fields.Monetary(
        string="Valor de Cŕedito", readonly=True, states=STATE
    )
    icms_aliquota_credito = fields.Float(
        string="% de Crédito", digits="Account", readonly=True, states=STATE
    )
    ...
    icms_st_aliquota_mva = fields.Float(
        string="% MVA", digits="Account", readonly=True, states=STATE
    )
    icms_st_aliquota = fields.Float(
        string="Alíquota", digits="Account", readonly=True, states=STATE
    )
    icms_st_base_calculo = fields.Monetary(
        string="Base ICMS ST", readonly=True, states=STATE
    )
    icms_st_aliquota_reducao_base = fields.Float(
        string="% Redução Base ST", digits="Account", readonly=True, states=STATE
    )
    icms_st_valor = fields.Monetary(string="Valor ICMS ST", readonly=True, states=STATE)

    icms_valor_original_operacao = fields.Float(
        string="ICMS da Operação", digits="Account", readonly=True, states=STATE
    )
    icms_aliquota_diferimento = fields.Float(
        string="% Diferimento", digits="Account", readonly=True, states=STATE
    )
    ...
    ipi_aliquota = fields.Float(
        string="Alíquota IPI", digits="Account", readonly=True, states=STATE
    )
    ipi_base_calculo = fields.Monetary(string="Base IPI", readonly=True, states=STATE)
    ipi_reducao_bc = fields.Float(
        string="% Redução Base", digits="Account", readonly=True, states=STATE
    )
    ipi_valor = fields.Monetary(string="Valor IPI", readonly=True, states=STATE)

    # ----------- II ----------------------
    ii_base_calculo = fields.Monetary(string="Base II", readonly=True, states=STATE)
    ii_aliquota = fields.Float(
        string="Alíquota II", digits="Account", readonly=True, states=STATE
    )
    ...
    ii_valor_iof = fields.Monetary(string="IOF", readonly=True, states=STATE)

    # ------------ PIS ---------------------
    pis_cst = fields.Selection(
        CST_PIS_COFINS, string="CST Pis", readonly=True, states=STATE
    )
    pis_aliquota = fields.Float(
        string="Alíquota PIS", digits="Account", readonly=True, states=STATE
    )
    pis_base_calculo = fields.Monetary(string="Base PIS", readonly=True, states=STATE)
    pis_valor = fields.Monetary(string="Valor PIS", readonly=True, states=STATE)
    pis_valor_retencao = fields.Monetary(
    ...
    cofins_cst = fields.Selection(
        CST_PIS_COFINS, string="CST COFINS", readonly=True, states=STATE
    )
    cofins_aliquota = fields.Float(
        string="Alíquota COFINS", digits="Account", readonly=True, states=STATE
    )
    cofins_base_calculo = fields.Monetary(
        string="Base COFINS", readonly=True, states=STATE
    )
    cofins_valor = fields.Monetary(string="Valor COFINS", readonly=True, states=STATE)
    cofins_valor_retencao = fields.Monetary(
    ...
    iss_aliquota = fields.Float(
        string="Alíquota ISS", digits="Account", readonly=True, states=STATE
    )
    iss_base_calculo = fields.Monetary(string="Base ISS", readonly=True, states=STATE)
    iss_valor = fields.Monetary(string="Valor ISS", readonly=True, states=STATE)
    iss_valor_retencao = fields.Monetary(
        string="Valor Retenção", readonly=True, states=STATE
    )

    # ------------ RETENÇÔES ------------
    csll_base_calculo = fields.Monetary(string="Base CSLL", readonly=True, states=STATE)
    csll_aliquota = fields.Float(
        string="Alíquota CSLL", digits="Account", readonly=True, states=STATE
    )
    csll_valor = fields.Monetary(string="Valor CSLL", readonly=True, states=STATE)
    csll_valor_retencao = fields.Monetary(
        string="Retenção CSLL", readonly=True, states=STATE
    )
    irpj_base_calculo = fields.Monetary(string="Base IRPJ", readonly=True, states=STATE)
    irpj_aliquota = fields.Float(
        string="Alíquota IRPJ", digits="Account", readonly=True, states=STATE
    )
    irpj_valor = fields.Monetary(string="Valor IRPJ", readonly=True, states=STATE)
    irpj_valor_retencao = fields.Monetary(
        string="Retenção IRPJ", readonly=True, states=STATE
    )
    irrf_base_calculo = fields.Monetary(string="Base IRRF", readonly=True, states=STATE)
    irrf_aliquota = fields.Float(
        string="Alíquota IRRF", digits="Account", readonly=True, states=STATE
    )
    irrf_valor = fields.Monetary(string="Valor IRRF", readonly=True, states=STATE)
    irrf_valor_retencao = fields.Monetary(
        string="Retenção IRRF", readonly=True, states=STATE
    )
    inss_base_calculo = fields.Monetary(string="Base INSS", readonly=True, states=STATE)
    inss_aliquota = fields.Float(
        string="Alíquota INSS", digits="Account", readonly=True, states=STATE
    )
    inss_valor = fields.Monetary(string="Valor INSS", readonly=True, states=STATE)
    inss_valor_retencao = fields.Monetary(
    ...
    icms_uf_dest = fields.Monetary(string="ICMS Destino", readonly=True, states=STATE)
    icms_fcp_uf_dest = fields.Monetary(string="Valor FCP", readonly=True, states=STATE)
    informacao_adicional = fields.Text(string="Informação Adicional")

    # =========================================================================
    # ICMS Retido anteriormente por ST
    # =========================================================================
    icms_substituto = fields.Monetary("ICMS Substituto", readonly=True, states=STATE)
    icms_bc_st_retido = fields.Monetary(
        "Base Calc. ST Ret.", readonly=True, states=STATE
    )
    icms_aliquota_st_retido = fields.Float("% ST Retido", readonly=True, states=STATE)
    icms_st_retido = fields.Monetary("ICMS ST Ret.", readonly=True, states=STATE)
